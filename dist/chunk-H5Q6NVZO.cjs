...
async function transformMacros({
    ...
    for (const macro of macros) {
      ...
      switch (true) {
        ...
        case ret instanceof String:
          ...
          break;
        case Boolean(ret):
          s.overwriteNode(node, JSON.stringify(ret));
          break;
        default:
          s.overwriteNode(node, "undefined");
          break;
      }
  ...
  function recordImports() {
    ...
    for (const node of program.body) {
      if (node.type === "ImportDeclaration" && node.importKind !== "type" && (node.attributes && checkImportAttributes(attrs, node.attributes) || _optionalChain([node, 'optionalAccess', _3 => _3.source, 'optionalAccess', _4 => _4.value, 'optionalAccess', _5 => _5.endsWith, 'optionalCall', _6 => _6(".macro")]))) {
        s.removeNode(node);
